                ...
                for (auto pB = init_subB; pB < alB; ++pB)
                {
                    if( dist(pA,pB) < 0.2 ) 
                    {
                        ...
                        if(pA->qID<=pB->qID)
                        {
                            ...
                            prod_buffer[ipb]=pA->qID;
                            prod_buffer[ipb+1]=pB->qID;
                            ipb += 2;
                        } 
                        else
                        {
                            // ++countingmap[ pB->qID ][ pA->qID ];
                            prod_buffer[ipb]=pB->qID;
                            prod_buffer[ipb+1]=pA->qID;
                            ipb += 2;
                        } 
                    }                    
                }
